...
<script type="text/javascript">
	...
	function changeDept(){
		window.location.href="teacher/addStudentClass.html?classID="+${request.schoolClass.id}+"&deptID="+$("#selectType").val();
	}
	function selectAll(){
		if($("#sa").is(':checked')){ 
...
<div class="main">
	<div class="rightpart col-md-10 col-md-offset-1">
		  <div class="boxtitle">
	  		<h5><b>添加学生到${request.schoolClass.className}</b>
	  		<s:if test="#request.deptMap!=null">
			<s:select id="selectType" onchange="changeDept()" label="未分班学生" list="#request.deptMap" listKey="key" listValue="value" value="#request.deptID" headerKey="0" headerValue="--"></s:select>
			</s:if>
	  		</h5>
	  	  </div>
	  	  <div class="showDesc">
	  	  <!-- dataTable -->
	  	  <form action="teacher/divideIntoClass.html" method="post">
	  	  <input name="classID" type="hidden" value="${request.schoolClass.id}">
	  	  <table class="classTable table table-hover display">
	  	  	<s:if test="#request.otherStudents!=null&&#request.otherStudents.size()==0">
				<div class="NotExist">
	    			<h1 align="center">:(</h1>
			    	<p align="center">该学院没有相关的学生信息</p>
			...
			<tr>
			    ...
			    <th>学校</th>
			    <th>学院</th>
			    <th><input id="sa" onclick="selectAll()" type="checkbox" value="${student.id}"/></th>
			</tr>
			</thead>
			<tbody>
			<s:iterator value="#request.otherStudents" var="student" status="status">
			<tr>
	  	  ...
	  	  <input class="btn btn-sm btn-primary col-md-3" type="submit" value="添 &nbsp;&nbsp;加">
	  	  </form>
	  	  <!-- dataTable.. -->
	  	  </div>	
	</div>
